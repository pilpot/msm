...
export class MsmGame implements IMsmGame {
	...
	constructor(
		...
		this.board = Array(1)
			.fill(null)
			.map(() => Array(this.colors).fill(null));
		this.resolution = Array(1)
			.fill(null)
			.map(() => Array(2).fill(0));
	...
	runGameSequence = async () => {
		...
		this.noConsoleOutput ||
			console.log('Starting answers count: ', this.allRemainingAnswers.length);

		while (this.attempts < this.rows && this.resolution[this.attempts][1] < this.columns) {
			if (
				!this.allRemainingAnswers.some((remaining) =>
					remaining.every((color, index) => color === this.answer[index])
				)
			) {
				throw new Error(
		...
		this.noConsoleOutput ||
			console.log('Game solved in ', this.attempts + 1, ' tries, result:', this.status);
	...
	public sortByLeastCommonColors(
