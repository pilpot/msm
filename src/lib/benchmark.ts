...
async function runBenchmark(numGames: number): Promise<void> {
	...
	console.log(
		`Columns: \x1b[33m${columnsCount}\x1b[0m, Colors: \x1b[33m${colors}\x1b[0m, Rows: \x1b[33m${boardRowsCount}\x1b[0m`
	);
	console.log(
		`Duplicates: \x1b[33m${
			allowDuplicates ? 'Yes' : 'No'
		}\x1b[0m, Guess Algorithm: \x1b[33m${guessAlgorithm}\x1b[0m`
	);
	console.log(
		babar(
			attemptsCountArray.map((count, index) => [index, count]),
			{ color: 'green', caption: `Count of games / Attemps` }
		)
	);
	...
	console.log(
		`The script used approximately \x1b[34m${Math.round(usedMemory * 100) / 100} MB\x1b[0m`
	);
}
