...
let mastermind: MastermindGame = {
	...
	board: Array(rows)
		.fill(null)
		.map(() => Array(colors).fill(null)),
	resolution: Array(rows)
		.fill(null)
		.map(() => Array(2).fill(0)),
...
async function outputBoard(): Promise<void> {
	...
	for (let i = 0; i < mastermind.rows; i++) {
		...
		mastermind.noConsoleOutput ||
			console.log(
				mastermind.board[i],
				mastermind.resolution[i],
				mastermind.remainingAnswersCount[i] || ''
			);
	}
...
async function setAnswer(answerProvided?: number[]): Promise<void> {
	...
	if (
		answerProvided &&
		answerProvided.some((num, i) => answerProvided.indexOf(num) !== i) &&
		!mastermind.allowDuplicates
	) {
		console.log(
			'Answer provided contains duplicates and allowDuplicates is false, answer provided: ',
			answerProvided
		);
	}
...
async function eliminateAnswers(boardRow: number): Promise<void> {
	...
	let totalColorsInBoardRow =
		mastermind.resolution[boardRow][0] + mastermind.resolution[boardRow][1];
...
async function runGameSequence(): Promise<void> {
	...
	while (
		mastermind.attempts < mastermind.rows &&
		mastermind.resolution[mastermind.attempts][1] < mastermind.columns
	) {
		if (
			!mastermind.allRemainingAnswers.some((remaining) =>
				remaining.every((color, index) => color === mastermind.answer[index])
			)
		) {
			throw new Error(
				'Answer has been erroneously eliminated from remaining answers, answer: ' +
					mastermind.answer
			);
		}
		// check for inclusion
...
async function makeATry(): Promise<boolean> {
	...
	// choose the remaining answer with the highest black pins count
		...
		mastermind.allRemainingAnswers = sortBySamePosition(
			mastermind.board[emptyRowIndex - 1],
			mastermind.allRemainingAnswers
		);
	...
	// choose the remaining answer with the less white pins count
		...
		mastermind.allRemainingAnswers = sortByLeastCommonColors(
			mastermind.board[emptyRowIndex - 1],
			mastermind.allRemainingAnswers
		);
		await setBoardRow(emptyRowIndex, mastermind.allRemainingAnswers[0]);
...
function sortByLeastCommonColors(
